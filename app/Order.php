...
class Order extends Model
{
    static public function GetOrders ($order_id = null, $status = null) {
        ...
        $products = DB::table('orders')
            ->leftJoin('partners', 'partners.id', '=', 'orders.partner_id') //partners partn ON partn.id = ord.partner_id
            ->leftJoin('order_products', 'order_products.order_id', '=', 'orders.id')   // order_products o_p ON o_p.order_id = ord.id
            ->leftJoin('products', 'products.id', '=', 'order_products.product_id')  //products prod ON prod.id = o_p.product_id
            ->leftJoin('vendors', 'vendors.id', '=', 'products.vendor_id')  //vendors vnd ON vnd.id = prod.vendor_id
            ->select('orders.*', 'partners.id as partner_id', 'partners.name as partner_name', 'vendors.name as vnd_name','products.name as prod_name','order_products.quantity','products.price')
